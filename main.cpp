...
std::string Format(std::string p_format, TParams && ... p_params)
{
    ...
    ((ToFmtString(ss, params.data(), index, manip_found, std::forward<TParams>(p_params))), ...);

    DoParameterReplace(p_format, index, params.data());
    (void)manip_found;
    return p_format;
}
...
int main(int, char **)
{
    std::cout << Format("%0", "Hello!") << '\n';
    std::cout << Format("%0", 12345) << '\n';
    std::cout << Format("%0 %0", "Hello!", "World!") << '\n';
    std::cout << Format("%2 %1", "world!", "Hello") << '\n';
    std::cout << Format("Hex: %0", std::hex, 12345) << '\n';
    std::cout << Format("Hex: ", std::hex, 12345) << '\n';
    std::cout << Format("Hello ", "World!") << '\n';
    std::cout << Format(std::hex , 12345) << '\n';
}    
